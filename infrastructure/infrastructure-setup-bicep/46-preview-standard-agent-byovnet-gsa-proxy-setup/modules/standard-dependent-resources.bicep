...
@description('Whether Cosmos DB should be zone redundant. Set to false in regions with limited AZ capacity.')
...
// Regions where Cosmos DB AZ capacity is constrained - redirect to a fallback region
var cosmosDbAZConstrainedRegions = ['southcentralus']
var cosmosDbRegion = contains(canaryRegions, location) ? 'westus' : (contains(cosmosDbAZConstrainedRegions, location) && !cosmosDBZoneRedundant ? 'westus' : location)
    ...
    locations: [
      {
        ...
        failoverPriority: 0
        isZoneRedundant: cosmosDBZoneRedundant
      }
    ]
